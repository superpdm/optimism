...
contract OPStackManager is ISemver, Initializable {
    ...
    // Step 1: Initialize blueprint contracts (must be called first)
    function initializePart1(Blueprints memory _blueprints) public initializer {
        blueprint = _blueprints;
    }

    // Step 2: Initialize SuperchainConfig and ProtocolVersions
    ...
    // Step 3: Initialize implementations for a release version
    function initializePart3(
        string memory _release,
        bool _isLatest,
        ImplementationSetter[] memory _setters
    )
        public
        reinitializer(3)
    {
