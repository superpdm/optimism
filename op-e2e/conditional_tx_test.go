...
var (
	...
	enableTxCondGethOption = func(ethCfg *ethconfig.Config, nodeCfg *node.Config) error {
		...
		ethCfg.RollupSequencerTxConditionalRateLimit = 1000 // not parsed from default CLI values so explicily set
		return nil
	}
)

func mkTransferTx(t *testing.T, cfg *SystemConfig, clnt *ethclient.Client) *types.Transaction {
...
func TestSendRawTransactionConditionalEnabled(t *testing.T) {
	InitParallel(t)
	cfg := DefaultSystemConfig(t)
	cfg.GethOptions[RoleSeq] = []geth.GethOption{enableTxCondGethOption}
...
func TestSendRawTransactionConditionalTxForwarding(t *testing.T) {
	...
	cfg.GethOptions[RoleVerif] = []geth.GethOption{enableTxCondGethOption}

	sys, err := cfg.Start(t)
	require.NoError(t, err, "Error starting up system")

	// wait for a couple l2 blocks to be created as conditionals are checked against older state
	verifClient := sys.NodeClient(RoleVerif)
	...
	_, err = wait.ForReceiptOK(context.Background(), sys.NodeClient(RoleSeq), tx.Hash())
	require.NoError(t, err)
}
